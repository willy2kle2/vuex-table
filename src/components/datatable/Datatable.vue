    ...
    <div class="data-container">
        ...
        <div class="checkbox-container" v-if="isColumnCheckable && !is_mobile">
            <div v-for="(column, index) in state.newColumns" :key="index">
                ...
                <input type="checkbox" class="datatable-checkbox checkbox-all inline-block" @change.sync="check_column(column)" :value="column.field" checked>
            </div>
        </div>
        <table class="table table-striped" v-if="Object.values(state.visibilities) !== false">
            <tbody>
            <tr class="headings" v-if="state.newColumns.length" @keydown.prevent.up="pressedArrow(-1)"
                ...
                <div class="datatable-checkbox">
                    <input type="checkbox" class="datatable-checkbox checkbox-all inline-block" :value="is_all_checked" :disabled="is_all_uncheckable"
                           @change="check_all"/>
                </div>
                    ...
                    <template v-for="(row, index) in search_data">
                        <div :key="index" :class="[rowClass(row, index), {'selected': is_row_checked(row)}]"
                            ...
                            <div class="row-container"  >
                                ...
                                <div v-for="(column, index) in state.newColumns" :key="index" class="cell" v-if="state.visibilities[column.field]">
                                    <slot :name="column.field" :index="index" :data="column.field" :label="column.label" :visible="state.visibilities[column.field]">
